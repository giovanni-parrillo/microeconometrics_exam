...
In the original paper, the authors considered different Regional Trade Agreements between Countries or group of Countries and looked at whether the inclusion of enviromental provisions aimed at reducing the enviromental impact of trade agreements was effective.
...
The rationale behind the study is that the implementation of RTAs lead to increased trade flows between the Countries involved, with particular effect on the trade of agricultural goods: higher demand for agricultural goods incentivates deforestation in order to convert to agriculture portions of land that were previously devoted to forest.
...
While we can't observe the variance and the bias of the estimator, we can estimate them through cross-validation dividing iteratively the sample in testing and training set.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Choosing_lambda_Lasso.pdf}
\caption{LASSO Lambda Selection via Cross-Validation}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Propensity_score_density.pdf}
\caption{Propensity Score Density}
\label{fig:propensity_score_density}
\end{figure}
...
In order to give causal interpretation to the estimated coefficients, we need some assumptions to hold.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Parallel_trends_averages.pdf}
\caption{Parallel Trends: Evolution of average log losses}
\label{fig:parallel_trends_averages}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Parallel_trends_event_study.pdf}
\caption{Parallel Trends: Coefficients from events study}
\label{fig:parallel_trends_event_study}
\end{figure}
...
As we can see, the main result of this model estimation is that RTAs without enviromental provisions lead to an increase in forest loss, but enviromental provisions are quite effective in mitigating this effect.
That result is obtained by accounting for endogeneity through propensity score matching, while the results are quite different when the entire dataset is considered without matching (and therefore, without accounting for heterogeneity).

We can also observe that the standard errors for the entire dataset are quite different from the ones obtained in the paper: we attribute it to the possibility that the sparse cross-cluster correlation methodology has been employed only in the matched sample in the original paper, while we applied it to both samples.
...
\subsubsection*{Extensions based on country-level forest loss}

The first extension is to use a subset of of country-level forest loss across agreement signatories, dividing countries in high-risk and low-risk ones: in particular, they consider tropical and non-tropical forest loss, forest loss in developed and developing countries, and forest loss in countries within the upper quartile of global biodiversity.
...
We were able to replicate their analysis, obtaining similar results: in particular, the effects of RTAs with or without enviromental provisions are significant only in high-risk Countries, while they are ambigous in low-risk Countries.
\raggedright
\vspace{2em}

\subsubsection*{Analyzing mechanisms}
\justifying
The authors of the original paper then proceed to study the effect of RTAs and enviromental provisions on other outcome variables: the rationale is to study which are the mechanisms through which RTAs and enviromental provisions affect deforestation.

They focus on developing and tropical Countries, as these are groups of Countries where the effect of RTAs and enviromental provisions have been found to be more significant.
...
As we can see from the tables above, the most significant effects are on the harvest area and harvest quantity: that is consistent with the aforementioned mechanism of deforestation driven by agricultural land expansion.

Another aspect investigated by the authors is the effect of RTAs and enviromental provisions on government involvement in the agricultural and forestry sectors, which is shown in the next tables.
...
Quite counterintuitively, if we consider the effect on forest loss, the presence of enforcement mechanisms seems to reduce the effectiveness of environmental provisions.

\newpage

\section*{Country-level analysis}

Finally, the authors of the original paper perform a country-level analysis, where they estimate the effect of RTAs and enviromental provisions on deforestation at the country level.

The model estimated is the following:
\begin{align*}
    y_{it} & = \delta_{LR-} \mathds{1}[{RTA}_{(<-3),it}] + \sum_{s=-3, s \neq -1}^{3} \delta_s \mathds{1}[ {RTA}_{s,it}] + \delta_{LR+} \mathds{1}[{RTA}_{(>3),it}] \\
    & + \xi_{LR-}\mathds{1}[enviro_{(<-3),it}]+ \sum_{s=-3, s \neq -1}^{3} \xi_s \mathds{1}[ {enviro}_{s,it}] + \xi_{LR+} \mathds{1}[enviro_{(>3),it}]\\
   &  \alpha_i + \gamma_t + \varepsilon_{it}
\end{align*}

In the figure below, we present the estimated $\delta_s$ coefficients, which are represented by the control group line, and the estimated combined effect of having an RTA with enviromental provisions ($\delta_s + \xi_s$), represented by the treatment group line.

\vspace{2em}

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{Countryeffects.pdf}
\caption{Country-Level Effects of Enviromental Provisions on Deforestation}
\label{fig:parallel_trends_averages}
\end{figure}
